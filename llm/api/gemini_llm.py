    ...
    async def _stream_user_message_to_llm(self, user_message: str, image_urls: Optional[List[str]] = None, **kwargs) -> AsyncGenerator[ChunkResponse, None]:
        ...
        try:
            ...
            async for chunk in stream:
                try:
                    # This is the line that causes the error. We'll wrap it.
                    chunk_text = chunk.text
                except (ValueError, AttributeError):
                    # This block will execute if the chunk has no text.
                    # This happens with tool calls or safety flags.
                    logger.warning("Skipping a streaming chunk from Gemini because it did not contain any text. This is expected when the model attempts a tool call.")
                    continue

                if chunk_text:
                    accumulated_content += chunk_text
                    yield ChunkResponse(content=chunk_text, is_complete=False, usage=None)
