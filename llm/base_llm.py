...
from typing import List, Optional, AsyncGenerator, Type, Dict, Union, TYPE_CHECKING
import logging
...
class BaseLLM(ABC):
    ...
    def configure_tools(self, tools: List['BaseTool']):
        """
        Configures the tools for the LLM instance. This should be called
        by the agent framework after the LLM is initialized and tools are known.
        """
        from autobyteus.tools.base_tool import BaseTool
        if not all(isinstance(t, BaseTool) for t in tools):
            raise TypeError("All items in tools list must be of type BaseTool")

        self.tools = tools
        logging.info(f"Configuring LLM '{self.model.name}' with {len(self.tools)} tools.")
        self._reconfigure_model_with_tools()

    @abstractmethod
    def _reconfigure_model_with_tools(self):
        """
        Abstract method for subclasses to reconfigure their underlying model instance
        with the tools provided via `configure_tools`. This is where native tool
        configuration for specific LLM providers should happen.
        """
        pass
