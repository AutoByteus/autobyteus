...
class AgentGroup:
    def __init__(self,
        ...
        # ... (__init__ body remains the same) ...
    ...
    def _initialize_agents(self):
        ...
        for def_name, original_definition in self._agent_definitions_map.items():
            ...
            auto_execute_tools: bool = runtime_overrides.get("auto_execute_tools", True) # RENAMED KEY
            if llm_model_name_for_agent is None:
                logger.warning(f"LLM model name not specified for agent definition '{def_name}'.")
            ...
            try:
                agent_runtime = self.agent_factory.create_agent_runtime(agent_id=agent_id, definition=effective_definition, llm_model_name=llm_model_name_for_agent,
                                                                    ...
                                                                    custom_tool_config=tool_config_for_factory, auto_execute_tools=auto_execute_tools) # RENAMED PARAM
    ...
    async def start(self):
        ...
        try:
            for agent in self.agents:
                if not agent.is_running: agent.start()
    ...
    def is_running(self) -> bool:
        # ... (property body remains the same) ...
