...
class WorkerEventDispatcher:
    ...
    def __init__(self,
        ...
        self.status_manager: 'AgentStatusManager' = status_manager
    ...
    async def dispatch(self, event: BaseEvent, context: 'AgentContext') -> None: # pragma: no cover
        ...
        if handler:
            ...
            current_status_before_dispatch = context.current_status

            if current_status_before_dispatch == AgentStatus.IDLE:
                if isinstance(event, (UserMessageReceivedEvent, InterAgentMessageReceivedEvent)):
                    await self.status_manager.notify_processing_input_started(trigger_info=type(event).__name__)
            
            if isinstance(event, LLMUserMessageReadyEvent):
                if current_status_before_dispatch not in [AgentStatus.AWAITING_LLM_RESPONSE, AgentStatus.ERROR]:
                    await self.status_manager.notify_awaiting_llm_response()
            elif isinstance(event, PendingToolInvocationEvent):
                if not context.auto_execute_tools:
                    await self.status_manager.notify_tool_execution_pending_approval(event.tool_invocation)
                else: 
                    await self.status_manager.notify_tool_execution_started(event.tool_invocation.name)
                ...
                await self.status_manager.notify_tool_execution_resumed_after_approval(
                 ...
                 if context.current_status == AgentStatus.EXECUTING_TOOL: 
                    await self.status_manager.notify_processing_tool_result(event.tool_name)
            elif isinstance(event, LLMCompleteResponseReceivedEvent):
                if context.current_status == AgentStatus.AWAITING_LLM_RESPONSE:
                ...
                if isinstance(event, LLMCompleteResponseReceivedEvent):
                    if context.current_status == AgentStatus.ANALYZING_LLM_RESPONSE and \
                       ...
                       context.input_event_queues.tool_invocation_request_queue.empty():
                           await self.status_manager.notify_processing_complete_and_idle()

            except Exception as e: 
                error_details = traceback.format_exc()
                error_msg = f"WorkerEventDispatcher '{agent_id}' error handling '{event_class_name}' with {handler_class_name}: {e}"
                logger.error(error_msg, exc_info=True)
                await self.status_manager.notify_error_occurred(error_msg, error_details) 
