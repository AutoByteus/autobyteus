...
2.  **Polyglot Support**: A skill folder can contain Python scripts, Java JARs, Bash scripts, etc. To execute a script, the agent constructs the absolute path and calls the appropriate tool (e.g., `bash_executor` for shell scripts, or uses Python interpreter for `.py` files).
