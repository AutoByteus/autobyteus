...
class WebPageScreenshotTaker(BaseTool, UIIntegrator):
    ...
    def __init__(self, config: Optional[ToolConfig] = None):
        ...
        if config:
            ...
            image_format_str = config.get('image_format', 'png')
            if image_format_str.lower() not in ['png', 'jpeg']:
                logger.warning(f"Invalid image_format '{image_format_str}'. Defaulting to 'png'.")
                self.image_format = "png"
            else:
                self.image_format = image_format_str.lower()

        logger.debug(f"WebPageScreenshotTaker initialized. Full_page: {self.full_page}, Format: {self.image_format}")
    ...
    async def _execute(self, context: 'AgentContext', **kwargs) -> Any:
        ...
        Args:
            ...
            **kwargs: Keyword arguments containing the URL and file path. The URL should be specified as 'url', 
                      and the file path as 'file_path'.
        ...
        logger.info(f"Agent '{context.agent_id}' taking screenshot of URL: {url}, saving to: {file_path}")
        await self.initialize()
        try:
            # Ensure directory for file_path exists
            os.makedirs(os.path.dirname(file_path), exist_ok=True)

            await self.page.goto(url, wait_until="networkidle") # Wait for network to be idle
            await self.page.screenshot(path=file_path, full_page=self.full_page, type=self.image_format) # type: ignore
            
            absolute_path = os.path.abspath(file_path)
            logger.info(f"Agent '{context.agent_id}' screenshot saved to {absolute_path}")
