...
class CreateFullPresentationTool(BaseTool):
    ...
    def get_argument_schema(self) -> ParameterSchema:
...
async def main(args):
    ...
    try:
        ...
        await registrar.list_and_register_remote_tools("google-slides-mcp")
        remote_tools = [tool for tool in default_tool_registry.get_all_tools() if tool.get_name().startswith("gslides_")]
        if not remote_tools:
