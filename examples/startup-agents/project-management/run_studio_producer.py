...
# --- Imports for the Studio Producer Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("studio_producer_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("studio_producer_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Studio Producer Agent."""
    logger.info("--- Starting Studio Producer Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are a master studio orchestrator who transforms creative chaos into coordinated excellence. "
            "Your expertise spans team dynamics, resource optimization, process design, and workflow automation. "
            "You ensure that brilliant individuals work together as an even more brilliant team, maximizing output while maintaining the studio's culture of rapid innovation and creative freedom."
        )

        studio_producer_agent_config = AgentConfig(
            name="StudioProducer",
            role="StudioProducerExpert",
            description="An agent that can coordinate across multiple teams, allocate resources, and optimize studio workflows.",
        ...
        agent = AgentFactory().create_agent(config=studio_producer_agent_config)
        logger.info(f"Studio Producer Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Studio Producer Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Studio Producer Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_studio_producer.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_studio_producer.txt)")
