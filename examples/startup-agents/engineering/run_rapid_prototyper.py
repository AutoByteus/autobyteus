...
# --- Imports for the Rapid Prototyper Agent Example ---
try:
    ...
    # TODO: Import MultiEdit, Glob, Task tools
...
# --- Logging Setup ---
logger = logging.getLogger("rapid_prototyper_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("rapid_prototyper_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Rapid Prototyper Agent."""
    logger.info("--- Starting Rapid Prototyper Agent Example ---")
    ...
    try:
        tools_for_agent = [
            ...
            file_writer,
            file_reader,
            # TODO: Add MultiEdit, Glob, Task tools
        ]
        ...
        system_prompt = (
            "You are an elite rapid prototyping specialist who excels at transforming ideas into functional applications at breakneck speed. "
            "Your expertise spans modern web frameworks, mobile development, API integration, and trending technologies. "
            "You embody the studio's philosophy of shipping fast and iterating based on real user feedback."
        )

        rapid_prototyper_agent_config = AgentConfig(
            name="RapidPrototyper",
            role="RapidPrototyperExpert",
            description="An agent that can quickly create a new application prototype, MVP, or proof-of-concept.",
        ...
        agent = AgentFactory().create_agent(config=rapid_prototyper_agent_config)
        logger.info(f"Rapid Prototyper Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Rapid Prototyper Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Rapid Prototyper Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_rapid_prototyper.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_rapid_prototyper.txt)")
