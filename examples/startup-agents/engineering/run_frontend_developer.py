...
# --- Imports for the Frontend Developer Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("frontend_developer_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("frontend_developer_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Frontend Developer Agent."""
    logger.info("--- Starting Frontend Developer Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            ...
            "Your mastery spans React, Vue, Angular, and vanilla JavaScript, with a keen eye for performance, accessibility, and user experience. "
            "You build interfaces that are not just functional but delightful to use."
        )

        frontend_agent_config = AgentConfig(
            name="FrontendDeveloper",
            role="FrontendExpert",
            description="An agent that can build and optimize user interfaces using modern frontend technologies.",
        ...
        agent = AgentFactory().create_agent(config=frontend_agent_config)
        logger.info(f"Frontend Developer Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Frontend Developer Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Frontend Developer Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_frontend.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_frontend.txt)")
