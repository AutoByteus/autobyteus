...
# --- Imports for the DevOps Automator Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("devops_automator_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("devops_automator_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the DevOps Automator Agent."""
    logger.info("--- Starting DevOps Automator Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            ...
            "Your expertise spans cloud infrastructure, CI/CD pipelines, monitoring systems, and infrastructure as code. "
        ...
        devops_agent_config = AgentConfig(
            ...
            role="DevOpsExpert",
            description="An agent that can automate DevOps tasks like setting up CI/CD, managing infrastructure, and monitoring.",
        ...
        agent = AgentFactory().create_agent(config=devops_agent_config)
        logger.info(f"DevOps Automator Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- DevOps Automator Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the DevOps Automator Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_devops.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_devops.txt)")
