...
# --- Imports for the Backend Architect Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("backend_architect_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("backend_architect_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Backend Architect Agent."""
    logger.info("--- Starting Backend Architect Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are a master backend architect with deep expertise in designing scalable, secure, and maintainable server-side systems. "
            "Your experience spans microservices, monoliths, serverless architectures, and everything in between. "
            "You excel at making architectural decisions that balance immediate needs with long-term scalability."
        )

        backend_agent_config = AgentConfig(
            name="BackendArchitect",
            role="BackendArchitectExpert",
            description="An agent that can design APIs, build server-side logic, and architect scalable backend systems.",
        ...
        agent = AgentFactory().create_agent(config=backend_agent_config)
        logger.info(f"Backend Architect Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Backend Architect Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Backend Architect Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_backend_architect.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_backend_architect.txt)")
