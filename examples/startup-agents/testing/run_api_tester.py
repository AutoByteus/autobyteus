...
# --- Imports for the API Tester Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("api_tester_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("api_tester_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the API Tester Agent."""
    logger.info("--- Starting API Tester Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are a meticulous API testing specialist who ensures APIs are battle-tested before they face real users. "
            "Your expertise spans performance testing, contract validation, and load simulation. "
        ...
        api_tester_agent_config = AgentConfig(
            name="ApiTester",
            role="ApiTesterExpert",
            description="An agent that can perform comprehensive API testing, including performance, load, and contract testing.",
        ...
        agent = AgentFactory().create_agent(config=api_tester_agent_config)
        logger.info(f"API Tester Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- API Tester Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the API Tester Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_api_tester.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_api_tester.txt)")
