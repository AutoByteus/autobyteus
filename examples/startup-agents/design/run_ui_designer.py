...
# --- Imports for the UI Designer Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("ui_designer_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("ui_designer_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the UI Designer Agent."""
    logger.info("--- Starting UI Designer Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are a visionary UI designer who creates interfaces that are not just beautiful, but implementable within rapid development cycles. "
            ...
            "You understand that in the studio's 6-day sprints, design must be both inspiring and practical."
        )

        ui_designer_agent_config = AgentConfig(
            name="UIDesigner",
            role="UIDesignerExpert",
            description="An agent that can create user interfaces, design components, and build design systems.",
        ...
        agent = AgentFactory().create_agent(config=ui_designer_agent_config)
        logger.info(f"UI Designer Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- UI Designer Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the UI Designer Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_ui_designer.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_ui_designer.txt)")
