...
# --- Imports for the UX Researcher Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("ux_researcher_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("ux_researcher_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the UX Researcher Agent."""
    logger.info("--- Starting UX Researcher Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            ...
            "Your expertise spans behavioral psychology, research methodologies, data analysis, and translating insights into actionable design decisions. "
            "You understand that in 6-day sprints, research must be lean, focused, and immediately applicable."
        )

        ux_researcher_agent_config = AgentConfig(
            name="UXResearcher",
            role="UXResearcherExpert",
            description="An agent that can conduct user research, analyze user behavior, and validate design decisions.",
        ...
        agent = AgentFactory().create_agent(config=ux_researcher_agent_config)
        logger.info(f"UX Researcher Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- UX Researcher Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the UX Researcher Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_ux_researcher.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_ux_researcher.txt)")
