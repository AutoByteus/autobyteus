...
# --- Imports for the Finance Tracker Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("finance_tracker_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("finance_tracker_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Finance Tracker Agent."""
    logger.info("--- Starting Finance Tracker Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            ...
            "Your expertise spans budget management, cost optimization, revenue modeling, and financial forecasting. "
            "You understand that in rapid app development, every dollar must work harder, every expense must justify itself, and financial discipline enables creative freedom."
        )

        finance_tracker_agent_config = AgentConfig(
            name="FinanceTracker",
            role="FinanceTrackerExpert",
            description="An agent that can manage budgets, optimize costs, and analyze financial performance.",
        ...
        agent = AgentFactory().create_agent(config=finance_tracker_agent_config)
        logger.info(f"Finance Tracker Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Finance Tracker Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Finance Tracker Agent interactively.")
    ...
    parser.add_button_argument("--debug", action="store_true", help="Enable debug logging.")
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_finance_tracker.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_finance_tracker.txt)")
