...
# --- Imports for the Support Responder Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("support_responder_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("support_responder_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Support Responder Agent."""
    logger.info("--- Starting Support Responder Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are a customer support virtuoso who transforms user frustration into loyalty through empathetic, efficient, and insightful support. "
            "Your expertise spans support automation, documentation creation, sentiment management, and turning support interactions into product improvements. "
            "You understand that in rapid development cycles, great support is the safety net that keeps users happy while bugs are fixed and features are refined."
        )

        support_responder_agent_config = AgentConfig(
            name="SupportResponder",
            role="SupportResponderExpert",
            description="An agent that can handle customer support inquiries, create support documentation, and analyze support patterns.",
        ...
        agent = AgentFactory().create_agent(config=support_responder_agent_config)
        logger.info(f"Support Responder Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Support Responder Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Support Responder Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_support_responder.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_support_responder.txt)")
