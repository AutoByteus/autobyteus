...
# --- Imports for the Infrastructure Maintainer Agent Example ---
try:
    ...
    # TODO: Import MultiEdit, WebSearch, Grep tools
...
# --- Logging Setup ---
logger = logging.getLogger("infrastructure_maintainer_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("infrastructure_maintainer_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Infrastructure Maintainer Agent."""
    logger.info("--- Starting Infrastructure Maintainer Agent Example ---")
    ...
    try:
        tools_for_agent = [
            ...
            file_writer,
            file_reader,
            # TODO: Add MultiEdit, WebSearch, Grep tools
        ]
        ...
        system_prompt = (
            "You are a infrastructure reliability expert who ensures studio applications remain fast, stable, and scalable. "
            "Your expertise spans performance optimization, capacity planning, cost management, and disaster prevention. "
            "You understand that in rapid app development, infrastructure must be both bulletproof for current users and elastic for sudden growth—while keeping costs under control."
        )

        infrastructure_maintainer_agent_config = AgentConfig(
            name="InfrastructureMaintainer",
            role="InfrastructureMaintainerExpert",
            description="An agent that can monitor system health, optimize performance, and ensure infrastructure reliability.",
        ...
        agent = AgentFactory().create_agent(config=infrastructure_maintainer_agent_config)
        logger.info(f"Infrastructure Maintainer Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Infrastructure Maintainer Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Infrastructure Maintainer Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_infrastructure_maintainer.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_infrastructure_maintainer.txt)")
