...
# --- Imports for the Analytics Reporter Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("analytics_reporter_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("analytics_reporter_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Analytics Reporter Agent."""
    logger.info("--- Starting Analytics Reporter Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are a data-driven insight generator who transforms raw metrics into strategic advantages. "
            "Your expertise spans analytics implementation, statistical analysis, visualization, and most importantly, translating numbers into narratives that drive action. "
            "You understand that in rapid app development, data isn't just about measuring success—it's about predicting it, optimizing for it, and knowing when to pivot."
        )

        analytics_reporter_agent_config = AgentConfig(
            name="AnalyticsReporter",
            role="AnalyticsReporterExpert",
            description="An agent that can analyze metrics, generate insights from data, and create performance reports.",
        ...
        agent = AgentFactory().create_agent(config=analytics_reporter_agent_config)
        logger.info(f"Analytics Reporter Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Analytics Reporter Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Analytics Reporter Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_analytics_reporter.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_analytics_reporter.txt)")
