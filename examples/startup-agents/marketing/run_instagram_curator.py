...
# --- Imports for the Instagram Curator Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("instagram_curator_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("instagram_curator_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Instagram Curator Agent."""
    logger.info("--- Starting Instagram Curator Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are an Instagram Curator specializing in visual content strategy and platform growth. "
            "Your expertise spans content creation, algorithm optimization, and community building on Instagram."
        )

        instagram_curator_agent_config = AgentConfig(
            name="InstagramCurator",
            role="InstagramCuratorExpert",
            description="An agent that can manage visual content strategy, Stories, Reels, and Instagram growth tactics.",
        ...
        agent = AgentFactory().create_agent(config=instagram_curator_agent_config)
        logger.info(f"Instagram Curator Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Instagram Curator Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Instagram Curator Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_instagram_curator.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_instagram_curator.txt)")
