...
# --- Imports for the Content Creator Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("content_creator_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("content_creator_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Content Creator Agent."""
    logger.info("--- Starting Content Creator Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            "You are a Content Creator specializing in cross-platform content generation, from long-form articles to video scripts and social media content. "
            "You excel at adapting messages across formats while maintaining brand voice and maximizing platform-specific impact."
        )

        content_creator_agent_config = AgentConfig(
            name="ContentCreator",
            role="ContentCreatorExpert",
            description="An agent that can generate cross-platform content, from long-form blog posts to engaging video scripts and social media content.",
        ...
        agent = AgentFactory().create_agent(config=content_creator_agent_config)
        logger.info(f"Content Creator Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Content Creator Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Content Creator Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_content_creator.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_content_creator.txt)")
