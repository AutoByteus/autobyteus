...
# --- Imports for the Growth Hacker Agent Example ---
try:
...
# --- Logging Setup ---
logger = logging.getLogger("growth_hacker_agent_example")
...
def setup_logging(args: argparse.Namespace):
    ...
    class FormattedConsoleFilter(logging.Filter):
        def filter(self, record):
            if record.name.startswith("growth_hacker_agent_example") or record.name.startswith("autobyteus.cli"):
                return True
...
async def main(args: argparse.Namespace):
    """Main function to configure and run the Growth Hacker Agent."""
    logger.info("--- Starting Growth Hacker Agent Example ---")
    ...
    try:
        ...
        system_prompt = (
            ...
            "You combine marketing creativity with analytical rigor to identify and exploit growth opportunities that drive exponential business growth."
        )

        growth_hacker_agent_config = AgentConfig(
            name="GrowthHacker",
            role="GrowthHackerExpert",
            description="An agent that can design viral loops, run growth experiments, and optimize acquisition channels.",
        ...
        agent = AgentFactory().create_agent(config=growth_hacker_agent_config)
        logger.info(f"Growth Hacker Agent instance created: {agent.agent_id}")
    ...
    logger.info("--- Growth Hacker Agent Example Finished ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the Growth Hacker Agent interactively.")
    ...
    parser.add_argument("--agent-log-file", type=str, default="./agent_logs_growth_hacker.txt", 
                       help="Path to the log file for autobyteus.* library logs. (Default: ./agent_logs_growth_hacker.txt)")
