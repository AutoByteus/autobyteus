...
@pytest.fixture
def sample_def1_json() -> AgentDefinition:
    """Returns a sample AgentDefinition configured for JSON."""
    return AgentDefinition(
        name="TestAgent1Json",
        role="WorkerJson",
        description="Description for TestAgent1 Worker (JSON)",
        system_prompt="System prompt for TestAgent1 Worker (JSON).",
        tool_names=["tool_a", "tool_b"],
...
def sample_def2() -> AgentDefinition:
    ...
    return AgentDefinition(
        ...
        tool_names=["tool_c"],
        use_xml_tool_format=True # Explicitly True
    )
...
def test_register_single_definition(empty_registry: AgentDefinitionRegistry, sample_def1: AgentDefinition, sample_def1_json: AgentDefinition):
    ...
    assert empty_registry._definitions[expected_key].use_xml_tool_format is True

    empty_registry.register(sample_def1_json)
    expected_key_json = empty_registry._generate_key(sample_def1_json.name, sample_def1_json.role)
    assert len(empty_registry) == 2 # Now two distinct definitions
    assert expected_key_json in empty_registry._definitions
    assert empty_registry._definitions[expected_key_json] is sample_def1_json
    assert empty_registry._definitions[expected_key_json].use_xml_tool_format is False
...
def test_register_overwrite_definition(empty_registry: AgentDefinitionRegistry, sample_def1: AgentDefinition, caplog):
    ...
    new_def_same_key = AgentDefinition(
        ...
        tool_names=["tool_new"],
        use_xml_tool_format=False # Change the flag for overwrite test
    )
    expected_key = empty_registry._generate_key(sample_def1.name, sample_def1.role)
    ...
    assert empty_registry._definitions[expected_key].use_xml_tool_format is False
...
def test_contains_check(empty_registry: AgentDefinitionRegistry, sample_def1: AgentDefinition):
    ...
    assert ("TestAgent1", "NonExistentRole") not in empty_registry
    assert ("", "Worker") not in empty_registry
    assert ("TestAgent1", "") not in empty_registry
    ...
    assert ("OnlyName",) not in empty_registry # type: ignore
    assert (123, "RoleString") not in empty_registry # type: ignore
