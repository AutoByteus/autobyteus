...
# or defining minimal ones here if specific variations are needed.

@pytest.fixture
def mock_agent_context_for_runtime(agent_context): # Use the more complete fixture
...
class TestAgentRuntimeNew:
    ...
    async def test_stop_full_flow(self, mock_format_exc, agent_runtime_with_mocks: AgentRuntime):
        runtime = agent_runtime_with_mocks
        mock_worker_instance = runtime._mock_worker_instance
        ...
        # No output queues to shut down anymore
        # context.output_data_queues.graceful_shutdown.assert_awaited_once_with(timeout=ANY) 
    ...
    async def test_stop_when_worker_not_alive(self, agent_runtime_with_mocks: AgentRuntime):
        ...
        # No output queues
        # runtime.context.output_data_queues.graceful_shutdown.reset_mock()
        ...
        # No output queues
        # runtime.context.output_data_queues.graceful_shutdown.assert_not_awaited()
        runtime.context.llm_instance.cleanup.assert_not_awaited()
        runtime.phase_manager.notify_final_shutdown_complete.assert_called_once()


    def test_handle_worker_completion_success_graceful_stop(self, agent_runtime_with_mocks: AgentRuntime):
        runtime = agent_runtime_with_mocks
        mock_future = MagicMock(spec=concurrent.futures.Future)
        mock_future.result = MagicMock(return_value=None) 
        mock_future.exception = MagicMock(return_value=None)

        runtime._mock_worker_instance._stop_initiated = True
        ...
        with patch('autobyteus.agent.runtime.agent_runtime.logger') as mock_logger:
            ...
            successful_completion_logged = any(
                f"Worker thread completed successfully" in call[0][0] 
    ...
    @patch('autobyteus.agent.runtime.agent_runtime.traceback.format_exc')
    def test_handle_worker_completion_with_future_exception_unexpected_exit(self, mock_format_exception, agent_runtime_with_mocks: AgentRuntime):
        runtime = agent_runtime_with_mocks
        context = agent_runtime_with_mocks.context
        ...
        mock_future.exception = MagicMock(return_value=test_exception)

        mock_format_exception.return_value = ["Mocked", "Traceback", "Details"] 

        runtime._mock_worker_instance._stop_initiated = False 
        ...
        with patch('autobyteus.agent.runtime.agent_runtime.logger') as mock_logger:
            ...
            unexpected_exit_logged = any(
                f"Worker thread exited unexpectedly" in str(call[0][0])
                for call in mock_logger.error.call_args_list
            )
            assert unexpected_exit_logged

        runtime.phase_manager.notify_error_occurred.assert_called_once_with(
            "Worker thread exited unexpectedly.", "".join(["Mocked", "Traceback", "Details"])
        )
    ...
    def test_is_running_property(self, agent_runtime_with_mocks: AgentRuntime):
        ...
        runtime._worker = None # type: ignore
        assert not runtime.is_running
        runtime._worker = original_worker 


    async def test_stop_no_llm_instance(self, agent_runtime_with_mocks: AgentRuntime):
        runtime = agent_runtime_with_mocks
        mock_worker_instance = runtime._mock_worker_instance
        context = agent_runtime_with_mocks.context

        context.llm_instance = None 
        mock_worker_instance.is_alive.return_value = True 
        ...
        mock_worker_instance.stop.reset_mock()
        # No output queues
        # context.output_data_queues.graceful_shutdown.reset_mock()

        await runtime.stop(timeout=0.1)
        
        assert context.llm_instance is None
        
        runtime.phase_manager.notify_shutdown_initiated.assert_called_once()
        mock_worker_instance.stop.assert_awaited_once_with(timeout=0.1)
        # No output queues
        # context.output_data_queues.graceful_shutdown.assert_awaited_once()
        runtime.phase_manager.notify_final_shutdown_complete.assert_called_once()
