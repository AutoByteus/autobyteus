...
@pytest.fixture
def basic_task_plan() -> TaskPlan:
    """Provides a task plan with two independent tasks."""
    plan = TaskPlan(
        overall_goal="A basic test plan.",
        tasks=[
...
@pytest.fixture
def dependent_task_plan() -> TaskPlan:
    """
    Provides a task plan with dependencies:
    A -> (B, C)
    B -> D
    """
    plan = TaskPlan(
        overall_goal="A plan with dependencies.",
        tasks=[
...
# --- Test Initialization and Plan Loading ---
...
def test_initialization(task_board: InMemoryTaskBoard):
    ...
    assert task_board.current_plan is None
    assert not task_board.task_statuses
    assert not task_board._task_map

def test_load_task_plan_success(task_board: InMemoryTaskBoard, basic_task_plan: TaskPlan):
    """Tests that loading a plan correctly sets up the board and emits an event."""
    with patch.object(task_board, 'emit') as mock_emit:
        result = task_board.load_task_plan(basic_task_plan)
        
        assert result is True
        assert task_board.current_plan == basic_task_plan
        assert len(task_board.task_statuses) == 2
        
        task_one_id = next(t.task_id for t in basic_task_plan.tasks if t.task_name == "task_one")
        ...
        assert event_name[0] == EventType.TASK_BOARD_PLAN_PUBLISHED
        payload = kwargs['payload']
        assert isinstance(payload, TaskPlanPublishedEvent)
        assert payload.plan == basic_task_plan

def test_load_task_plan_failure_with_invalid_type(task_board: InMemoryTaskBoard):
    """Tests that loading an object that isn't a TaskPlan fails."""
    result = task_board.load_task_plan("not a plan")
    assert result is False

# --- Test Status Updates ---

def test_update_task_status_success(task_board: InMemoryTaskBoard, basic_task_plan: TaskPlan):
    """Tests that updating a status works correctly and emits an event."""
    task_board.load_task_plan(basic_task_plan)
    task_one_id = next(t.task_id for t in basic_task_plan.tasks if t.task_name == "task_one")
    ...
    with patch.object(task_board, 'emit') as mock_emit:
        ...
        mock_emit.assert_called_once()
        event_name, kwargs = mock_emit.call_args
        assert event_name[0] == EventType.TASK_BOARD_STATUS_UPDATED
        payload = kwargs['payload']
...
def test_get_runnable_no_plan_loaded(task_board: InMemoryTaskBoard):
    """Tests that no tasks are runnable if no plan is loaded."""
    assert task_board.get_next_runnable_tasks() == []

def test_get_runnable_no_dependencies(task_board: InMemoryTaskBoard, basic_task_plan: TaskPlan):
...
def test_get_runnable_with_dependencies_initial_state(task_board: InMemoryTaskBoard, dependent_task_plan: TaskPlan):
    """Tests that only tasks with no dependencies are initially runnable."""
    task_board.load_task_plan(dependent_task_plan)
    runnable = task_board.get_next_runnable_tasks()

    assert len(runnable) == 1
    assert runnable[0].task_name == "A"

def test_get_runnable_dependency_completed(task_board: InMemoryTaskBoard, dependent_task_plan: TaskPlan):
    """Tests that tasks become runnable once their dependencies are met."""
    task_board.load_task_plan(dependent_task_plan)
    task_a = next(t for t in dependent_task_plan.tasks if t.task_name == "A")
...
def test_get_runnable_multi_level_dependency_flow(task_board: InMemoryTaskBoard, dependent_task_plan: TaskPlan):
    """Tests the flow of runnable tasks through a multi-level dependency chain."""
    task_board.load_task_plan(dependent_task_plan)
    task_a = next(t for t in dependent_task_plan.tasks if t.task_name == "A")
    task_b = next(t for t in dependent_task_plan.tasks if t.task_name == "B")
...
def test_get_runnable_with_failed_dependency(task_board: InMemoryTaskBoard, dependent_task_plan: TaskPlan):
...
def test_get_runnable_when_task_in_progress(task_board: InMemoryTaskBoard, dependent_task_plan: TaskPlan):
...
def test_get_status_overview(task_board: InMemoryTaskBoard, basic_task_plan: TaskPlan):
    """Tests that the status overview is correct for both empty and loaded boards."""
    # Test on empty board
    overview_empty = task_board.get_status_overview()
    assert overview_empty["plan_id"] is None
    assert overview_empty["overall_goal"] is None
    ...
    assert overview_loaded["plan_id"] == basic_task_plan.plan_id
    assert overview_loaded["overall_goal"] == basic_task_plan.overall_goal
