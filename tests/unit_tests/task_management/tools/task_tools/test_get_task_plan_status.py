# file: autobyteus/tests/unit_tests/task_management/tools/task_tools/test_get_task_plan_status.py
import json
import pytest
...
from autobyteus.task_management.tools import GetTaskPlanStatus
...
def mock_team_context_with_board() -> AgentTeamContext:
    """Provides a mock AgentTeamContext with a mocked TaskPlan."""
    mock_context = Mock(spec=AgentTeamContext)
    mock_state = Mock(spec=AgentTeamRuntimeState)
    mock_state.task_plan = Mock(spec=InMemoryTaskPlan)
    mock_context.state = mock_state
    mock_context.team_id = "test_team"
    return mock_context

@pytest.mark.asyncio
@patch('autobyteus.task_management.tools.task_tools.get_task_plan_status.TaskPlanConverter.to_schema')
async def test_execute_success(mock_to_schema: MagicMock, tool_instance: GetTaskPlanStatus, mock_agent_context: AgentContext, mock_team_context_with_board: AgentTeamContext):
    ...
    mock_to_schema.assert_called_once_with(mock_team_context_with_board.state.task_plan)
...
@pytest.mark.asyncio
@patch('autobyteus.task_management.tools.task_tools.get_task_plan_status.TaskPlanConverter.to_schema')
async def test_execute_success_with_deliverables(mock_to_schema: MagicMock, tool_instance: GetTaskPlanStatus, mock_agent_context: AgentContext, mock_team_context_with_board: AgentTeamContext):
...
@pytest.mark.asyncio
@patch('autobyteus.task_management.tools.task_tools.get_task_plan_status.TaskPlanConverter.to_schema')
async def test_execute_with_no_tasks_on_board(mock_to_schema: MagicMock, tool_instance: GetTaskPlanStatus, mock_agent_context: AgentContext, mock_team_context_with_board: AgentTeamContext):
    ...
    assert result == "The task plan is currently empty. No tasks have been published."

@pytest.mark.asyncio
async def test_execute_no_team_context(tool_instance: GetTaskPlanStatus, mock_agent_context: AgentContext):
...
@pytest.mark.asyncio
async def test_execute_no_task_plan(tool_instance: GetTaskPlanStatus, mock_agent_context: AgentContext, mock_team_context_with_board: AgentTeamContext):
    """Tests failure when the task plan has not been initialized in the team's state."""
    # Arrange
