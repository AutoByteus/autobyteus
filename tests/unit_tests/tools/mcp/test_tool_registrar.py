...
def mock_dependencies():
    ...
    with patch('autobyteus.tools.mcp.tool_registrar.McpConfigService') as mock_config_service, \
         patch('autobyteus.tools.mcp.tool_registrar.ToolRegistry') as mock_tool_registry, \
         patch('autobyteus.tools.mcp.tool_registrar.McpServerInstanceManager') as mock_instance_manager:
        
        registrar = McpToolRegistrar()
        
        yield {
            "registrar": registrar,
            "config_service": mock_config_service.return_value,
            "tool_registry": mock_tool_registry.return_value,
            "instance_manager": mock_instance_manager.return_value,
        }

@pytest.mark.asyncio
async def test_reload_all_mcp_tools(mock_dependencies):
    ...
    with patch.object(registrar, '_fetch_tools_from_server', new_callable=AsyncMock) as mock_fetch:
        ...
        stale_tool_def.name = "stale_tool"
        ...
        assert registered_def.category == "server1"
        assert registered_def.metadata["mcp_server_id"] == "server1"

        # Assert provider-based schema handling
        assert callable(registered_def._argument_schema_provider)
        # Access the property to trigger the provider
        schema = registered_def.argument_schema
        assert isinstance(schema, ParameterSchema)
        assert schema.get_parameter("paramA") is not None
...
@pytest.mark.asyncio
async def test_register_server(mock_dependencies):
    ...
    with patch.object(registrar, '_fetch_tools_from_server', new_callable=AsyncMock) as mock_fetch:
        ...
        stale_tool_def = MagicMock(spec=ToolDefinition)
        stale_tool_def.name = "old_tool_for_target"
...
@pytest.mark.asyncio
async def test_load_and_register_server(mock_dependencies):
    ...
    with patch.object(registrar, 'register_server', new_callable=AsyncMock) as mock_register_server:
        # --- Act ---
...
@pytest.mark.asyncio
async def test_reload_all_mcp_tools_fetch_fails(mock_dependencies):
...
@pytest.mark.asyncio
async def test_list_remote_tools_previews_without_side_effects(mock_dependencies):
