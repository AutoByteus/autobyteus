...
def parser() -> AnthropicXmlToolUsageParser:
    # This parser is an alias for DefaultXmlToolUsageParser, so tests should reflect that behavior.
    return AnthropicXmlToolUsageParser()

def test_parse_single_tool_block(parser: AnthropicXmlToolUsageParser):
    # Arrange
    response_text = """
    I think I should use a tool.
    <tool name="get_stock_price" id="tool_123">
        <arguments>
            <arg name="ticker_symbol">GOOG</arg>
        </arguments>
    </tool>
    That is the tool I will use.
    """
...
def test_parse_multiple_separate_tool_blocks(parser: AnthropicXmlToolUsageParser):
    # Arrange
    response_text = """
...
def test_tool_with_no_arguments_tag(parser: AnthropicXmlToolUsageParser):
    # Arrange
...
def test_tool_with_empty_arguments_tag(parser: AnthropicXmlToolUsageParser):
    # Arrange
...
def test_parse_generates_id_if_missing(parser: AnthropicXmlToolUsageParser):
    # Arrange
    response_text = """
...
def test_no_tool_blocks(parser: AnthropicXmlToolUsageParser):
    # Arrange
...
def test_malformed_xml_is_ignored(parser: AnthropicXmlToolUsageParser):
...
def test_ignores_other_xml_tags(parser: AnthropicXmlToolUsageParser):
    # Arrange
    response_text = """
    <thinking>I should use a tool.</thinking>
    <tool name="my_tool"><arguments></arguments></tool>
    <result>OK</result>
    """
    response = CompleteResponse(content=response_text)
    # Act
    ...
    # Assert
    assert len(invocations) == 1
    assert invocations[0].name == "my_tool"

def test_handles_special_chars_in_args(parser: AnthropicXmlToolUsageParser):
    # Arrange
    response_text = """
...
def test_mixed_valid_and_truly_malformed_block(parser: AnthropicXmlToolUsageParser):
    # The new robust parser should skip the malformed block and find the valid one.
    response_text = """
...
def test_incomplete_or_invalid_tags_are_ignored(parser: AnthropicXmlToolUsageParser, text_fragment: str):
    """
...
def test_realistic_scenario_with_mixed_content(parser: AnthropicXmlToolUsageParser):
    # Arrange
    realistic_output = """
