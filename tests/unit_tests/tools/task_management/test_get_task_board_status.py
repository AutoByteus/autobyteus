# file: autobyteus/tests/unit_tests/tools/task_management/test_get_task_board_status.py
import json
import pytest
...
def test_definition():
    """Tests that the tool is correctly defined and registered."""
    definition = default_tool_registry.get_tool_definition(TOOL_NAME)
    assert definition is not None
    assert definition.name == TOOL_NAME
    assert "LLM-friendly JSON string" in definition.description
    assert definition.category == ToolCategory.TASK_MANAGEMENT
    assert definition.argument_schema is None

@pytest.mark.asyncio
@patch('autobyteus.task_management.tools.get_task_board_status.TaskBoardConverter.to_status_report')
async def test_execute_success(mock_to_status_report: MagicMock, tool_instance: GetTaskBoardStatus, mock_agent_context: AgentContext, mock_team_context_with_board: AgentTeamContext):
    ...
    mock_report = TaskStatusReport(overall_goal="mock goal", tasks=[])
    mock_to_status_report.return_value = mock_report
    
    # Act
    result = await tool_instance.execute(mock_agent_context)
    
    # Assert
    mock_to_status_report.assert_called_once_with(mock_team_context_with_board.state.task_board)
...
@pytest.mark.asyncio
@patch('autobyteus.task_management.tools.get_task_board_status.TaskBoardConverter.to_status_report')
async def test_execute_with_no_plan_loaded(mock_to_status_report: MagicMock, tool_instance: GetTaskBoardStatus, mock_agent_context: AgentContext, mock_team_context_with_board: AgentTeamContext):
    """Tests execution when the converter returns None (i.e., no plan is loaded)."""
    # Arrange
    mock_agent_context.custom_data["team_context"] = mock_team_context_with_board
    mock_to_status_report.return_value = None
