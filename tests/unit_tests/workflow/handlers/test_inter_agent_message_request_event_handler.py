...
@pytest.mark.asyncio
async def test_handle_success(handler: InterAgentMessageRequestEventHandler, event: InterAgentMessageRequestEvent, workflow_context: WorkflowContext, mock_agent: Agent):
    """
    ...
    workflow_context.team_manager.ensure_agent_is_ready.assert_awaited_once_with(event.recipient_name)
...
@pytest.mark.asyncio
async def test_handle_agent_not_found_or_failed_to_start(handler: InterAgentMessageRequestEventHandler, event: InterAgentMessageRequestEvent, workflow_context: WorkflowContext, mock_agent: Agent, caplog):
    """
    Tests the failure path where TeamManager returns None (agent not found or failed to start).
    """
    ...
    assert f"Recipient agent '{event.recipient_name}' not found or failed to start" in caplog.text
