...
async def test_run_success(workflow_context, mock_step_1, mock_step_2):
    ...
    status_manager = workflow_context.status_manager
    
    success = await bootstrapper.run(workflow_context, status_manager)

    assert success is True
    status_manager.notify_bootstrapping_started.assert_awaited_once()
    mock_step_1.execute.assert_awaited_once_with(workflow_context, status_manager)
    mock_step_2.execute.assert_awaited_once_with(workflow_context, status_manager)
