...
class _Summarizer(Summarizer):
    def summarize(self, traces):
        return CompactionResult(
            ...
            semantic_facts=[{"fact": "tool flow preserved", "confidence": 0.9}],
        )
...
@pytest.mark.asyncio
async def test_memory_compaction_includes_tool_tail(tmp_path):
    store = FileMemoryStore(base_dir=tmp_path, agent_id="agent_compact_tool_tail")
    ...
    # Old turn to compact
    old_turn = memory_manager.start_turn()
    store.add(
        [
            _trace(old_turn, 1, "user", "old user"),
            _trace(old_turn, 2, "assistant", "old assistant"),
        ]
    )

    # Tail turn with tool interaction
    tail_turn = memory_manager.start_turn()
    store.add(
        [
            _trace(tail_turn, 1, "user", "tail user"),
            _trace(tail_turn, 2, "tool_call", tool_name="write_file", tool_call_id="call_1", tool_args={"path": "x.txt"}),
            _trace(tail_turn, 3, "tool_result", tool_name="write_file", tool_call_id="call_1", tool_result="ok"),
            _trace(tail_turn, 4, "assistant", "tail assistant"),
        ]
    )
