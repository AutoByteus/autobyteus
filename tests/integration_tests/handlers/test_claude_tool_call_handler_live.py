...
def set_claude_env(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", os.getenv("ANTHROPIC_API_KEY", "YOUR_API_KEY"))

@pytest.fixture
def claude_llm(set_claude_env):
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if not api_key or api_key == "YOUR_API_KEY":
        pytest.skip("ANTHROPIC_API_KEY not set. Skipping tests.")
    
    # Use Sonnet 3.5 (or latest available mapped model)
    return ClaudeLLM(model=LLMModel['claude-4.5-sonnet'])

@pytest.mark.asyncio
async def test_claude_handler_integrates_with_live_stream(claude_llm):
    """
    Verifies that the ApiToolCallStreamingResponseHandler correctly processes
    a live stream from Claude (Anthropic), accumulating deltas and emitting events.
    """
    ...
    formatter = AnthropicJsonSchemaFormatter()
    tool_schema = formatter.provide(tool_def)
    
    # 3. Stream from LLM
    # Request a tool call
    user_message = LLMUserMessage(content="Write a python file named anthropic_test.py with content 'print(1)'")
    
    print("\nStarting Claude stream...")
    
    chunks_received = 0
    # Feed chunks to handler
    async for chunk in claude_llm._stream_user_message_to_llm(
        user_message,
        tools=[tool_schema]
    ):
        chunks_received += 1
    ...
    assert invocation.arguments["path"] == "anthropic_test.py"
