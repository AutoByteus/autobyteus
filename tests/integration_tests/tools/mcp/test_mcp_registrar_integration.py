...
# Refactored imports - McpConnectionManager is gone
...
expected_tools_details = [
    ...
    {"name": "batch_update_presentation", "description": "Apply a batch of updates to a Google Slides presentation", "params": [{"name": "presentationId", "type": ParameterType.STRING, "required": True}, {"name": "requests", "type": ParameterType.ARRAY, "required": True, "item_schema_type": "object"}, {"name": "writeControl", "type": ParameterType.OBJECT, "required": False}]},
...
def mcp_test_environment(google_slides_mcp_script_path):
    ...
    # The registrar is now self-contained with its handlers
    ...
    yield registrar  # Provide only the registrar to the test

    # Teardown: No more conn_manager.cleanup() needed. Just restore registry.
...
async def test_mcp_registrar_discovers_and_registers_google_slides_tools(mcp_test_environment):
    ...
    registrar = mcp_test_environment
