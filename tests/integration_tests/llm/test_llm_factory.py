...
def test_llm_initialization_with_custom_config(llm_factory, model_name):
    ...
    custom_config = LLMConfig(
        rate_limit=40, token_limit=8192
    )  # Replace with actual configuration parameters
    llm_instance = llm_factory.create_llm(model_name, custom_config=custom_config)
    assert (
        llm_instance.config == custom_config
    ), f"LLM instance for {model_name} should have the custom config."
...
def test_plugins_registration(llm_factory):
    ...
    for plugin_model in plugin_added_models:
        assert (
            plugin_model in models
        ), f"Plugin model {plugin_model} should be registered in the factory."
